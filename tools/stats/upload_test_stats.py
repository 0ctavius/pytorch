...
def summarize_test_cases(
    test_cases: List[Dict[str, Any]], pytest_parallel_times: Dict[Any, Any]
) -> List[Dict[str, Any]]:
    ...
    for key, val in ret:
        ...
        pytest_key = (key[5], key[2])
        if pytest_key in pytest_parallel_times:
            val["invoking_file_time"] = pytest_parallel_times[pytest_key]
        else:
            val["invoking_file_time"] = val["time"]
...
if __name__ == "__main__":
    ...
    upload_to_s3(
        ...
        "test_run_summary",
        summarize_test_cases(test_cases, pytest_parallel_times),
    )
